...
var d8 = debug8("electron-chrome-extensions:runtime");
...
var RuntimeAPI = class extends EventEmitter3 {
  ...
  setupPortHandlers() {
    ipcMain.on("crx-port-msg", (event, extensionId, portId, message) => {
    ...
    ipcMain.on("crx-port-disconnect", (event, extensionId, portId) => {
      const port = this.ports.get(portId);
      if (!port) return;
    ...
    ipcMain.on("crx-port-message-to-popup", (event, portId, message) => {
    ...
    ipcMain.on("crx-port-disconnect-from-bg", (event, portId) => {
  ...
  findBackgroundPage(extensionId) {
    const { webContents: webContents2 } = __require("electron");
...
var d9 = debug9("electron-chrome-extensions:router");
...
function resolvePreloadPath(modulePath) {
  try {
    return createRequire(import.meta.dirname).resolve("electron-chrome-extensions/preload");
  } catch (error) {
  ...
  return path4.join(import.meta.dirname, preloadFilename);
}
  ...
  setupSWScriptInterception() {
    ...
    try {
      ...
      session2.protocol.handle("chrome-extension", (request) => {
        ...
        const filePath = path4.join(extension.path, requestPath);
